...
{
    public partial class 奶酪系列 : Form
    {
        public 奶酪系列()
        {
            InitializeComponent();
        ...
        private void 奶酪系列_Load(object sender, EventArgs e)
        {
        ...
        void 讀取資料庫的方法()
        {
            ...
            con.Open();
            string str = "Select* from product where level=3;";
        ...
        {
            listView奶酪系列.Clear();
            listView奶酪系列.View = View.LargeIcon;  //大圖模式圖上字下
            //列表有一種，圖片有四種(llargeicon/smallicon圖字平行/list/Tile)
            imageList1.ImageSize = new Size(190,140);  //改變照片讀出大小
            listView奶酪系列.LargeImageList = imageList1;//就可顯示圖片
        ...
        private void lbl最下面著作權標示_Click(object sender, EventArgs e)
        {

        }

        private void listView奶酪系列_ItemActivate(object sender, EventArgs e)
        {
            int selID = (int)listView奶酪系列.SelectedIndices[0];   //取得所選項目的ID
            string 點選產品 = "";
            點選產品 = listName[selID];
            string 點選圖片 = "";
            點選圖片 = listpic[selID];
        ...
        private void 奶酪系列_Activated(object sender, EventArgs e)
        {
            if (Global.訂購總數 == 0)
            {
                lbl總數量.Visible = false;
        ...
        private void 奶酪系列_Deactivate(object sender, EventArgs e)
        {
            if (Global.訂購總數 == 0)
            {
                lbl總數量.Visible = false;
